...
class XiaomiMyBand:
    ...
    step_count = 0
    battery_level = 81
    medicamentos = 'paracetamol', 'ibuprofeno', 'insulina', 'furosemida', 'piroxicam', 'tolbutamida'
    id = 0
    ...
    def publish(self):
        ...
        #FIN BLOQUE ACELEROMETRO

        #BLOQUE MEDICAMENTOS
        time.sleep(1)

        message['simular_medicamento'] = self.simular_medicamento()
        message['id'] = str(self.id)
        message['datetime'] = self.simulate_datetime()
        message['producer'] = self.producer
        message['model'] = self.model
        message['hardware_version'] = self.hardware_version
        message['software_version'] = self.software_version
        message['hora']= self.simular_hora()
        # Se establece la conexión con el Distribuidor de Mensajes
        connection = pika.BlockingConnection(pika.ConnectionParameters(host='localhost'))
        # Se solicita un canal por el cuál se enviarán los signos vitales
        channel = connection.channel()
    ...
    def simular_medicamento(self):
        return self.medicamentos[random.randint(0, len(self.medicamentos)-1)]
